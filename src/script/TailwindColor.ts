  ...
  const all: any[] = [];
  for (const label of textSizes) all.push({
    label,
    kind: 17,
    insertText: label,
    detail: `Tailwind ${label}`,
    documentation: `Tailwind font size utility ${label}`,
    sortText: label
  });
// Export an array of Tailwind color utility suggestions for Monaco completion
// Generate and filter valid Tailwind color suggestions for Monaco
function createTailwindColorSuggestions() {
  const all = [];
  const colors = [
    ...
    "pink", "rose",
    // New in Tailwind v4
    "brown", "olive", "mint", "sage", "sand",
    "gold", "bronze", "tomato", "ruby", "crimson",
    "plum", "iris", "jade", "grass", "forest",
    "sea", "wave", "horizon", "lavender", "orchid"
  ];
  const shades = [50,100,200,300,400,500,600,700,800,900,950];
  // bg
  for (const color of colors) for (const shade of shades) all.push({
    label: `bg-${color}-${shade}`,
    kind: 17,
    insertText: `bg-${color}-${shade}`,
    detail: `Tailwind bg-${color}-${shade}`,
    documentation: `Tailwind background color ${color}-${shade}`,
    sortText: `bg-${color}-${shade}`
  });
  // text
  for (const color of colors) for (const shade of shades) all.push({
    label: `text-${color}-${shade}`,
    kind: 17,
    insertText: `text-${color}-${shade}`,
    detail: `Tailwind text-${color}-${shade}`,
    documentation: `Tailwind text color ${color}-${shade}`,
    sortText: `text-${color}-${shade}`
  });
  // border
  for (const color of colors) for (const shade of shades) all.push({
    label: `border-${color}-${shade}`,
    kind: 17,
    insertText: `border-${color}-${shade}`,
    detail: `Tailwind border-${color}-${shade}`,
    documentation: `Tailwind border color ${color}-${shade}`,
    sortText: `border-${color}-${shade}`
  });
  // Special
  const specials = [
    ...
    "bg-inherit", "text-inherit", "border-inherit",
    "bg-unset", "text-unset", "border-unset",
    "bg-initial", "text-initial", "border-initial",
    "bg-revert", "text-revert", "border-revert",
    "bg-revert-layer", "text-revert-layer", "border-revert-layer"
  ];
  for (const label of specials) all.push({
    label,
    kind: 17,
    insertText: label,
    detail: `Tailwind ${label}`,
    documentation: `Tailwind color utility ${label}`,
    sortText: label
  });
  // Opacity
  for (const opacity of [10,20,30,40,50,60,70,80,90]) {
    all.push({
      label: `bg-opacity-${opacity}`,
      kind: 17,
      insertText: `bg-opacity-${opacity}`,
      detail: `Tailwind bg-opacity-${opacity}`,
      documentation: `Sets background opacity to ${opacity}%`,
      sortText: `bg-opacity-${opacity}`
    });
    all.push({
      label: `text-opacity-${opacity}`,
      kind: 17,
      insertText: `text-opacity-${opacity}`,
      detail: `Tailwind text-opacity-${opacity}`,
      documentation: `Sets text opacity to ${opacity}%`,
      sortText: `text-opacity-${opacity}`
    });
    all.push({
      label: `border-opacity-${opacity}`,
      kind: 17,
      insertText: `border-opacity-${opacity}`,
      detail: `Tailwind border-opacity-${opacity}`,
      documentation: `Sets border opacity to ${opacity}%`,
      sortText: `border-opacity-${opacity}`
    });
  }
  // Filter: label string, insertText string, kind number, tidak kosong, tidak undefined
  const seen = new Set();
  return all.filter(item => {
    if (!item.label || typeof item.label !== 'string') return false;
    if (!item.insertText || typeof item.insertText !== 'string') return false;
    if (typeof item.kind !== 'number') return false;
...
export const tailwindColorSuggestions = createTailwindColorSuggestions();
